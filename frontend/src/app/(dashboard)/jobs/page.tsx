...
export default function ProjectsPage() {
 ...
 spread: 70,
 origin: { y: 0.6 },
 colors: ['#fd5321', '#001011', '#007aff', '#fcfbf8'],
 });
 }
 ...
 <p className="mt-2 text-[15px] text-storm/75">
 ...
 <div
 key={col.id}
 className={`flex h-full min-w-[300px] max-w-[340px] flex-1 flex-col pl-6 pr-3 ${i > 0 ? 'border-l border-mist' : ''}`}
 >
 <div className="flex items-baseline justify-between pb-4 pt-1">
 <div className="h-3 w-20 rounded bg-ink/10 animate-pulse" />
 <div className="h-4 w-4 rounded bg-ink/10 animate-pulse" />
 </div>
 <div className="flex flex-col gap-3 pt-1">
 {[1, 2, 3].map((j) => (
 <div key={j} className="h-[112px] rounded-2xl bg-parchment animate-pulse" />
 ))}
 </div>
 </div>
 ))}
 ...
 <div>
 <h3 className="mb-2 flex items-center gap-2 text-sm font-semibold text-storm/70">
 ...
 className="rounded-md border bg-parchment px-3 py-2 text-left text-sm transition-colors hover:bg-ink/[0.06]/50"
 >
 ...
 <p className="text-xs text-storm/70">No completed projects.</p>
 )}
 </div>
 </div>
 )}
 ...
 <div
 key={p.id}
 className="flex items-center gap-2 rounded-md border bg-parchment px-3 py-2 text-sm"
 >
 <button
 onClick={() => setSelectedProjectId(p.id)}
 ...
 <p className="text-xs text-storm/70">No lost projects.</p>
 )}
 </div>
 </div>
 )}
 ...
 <p className="text-xs text-storm/70">No cancelled projects.</p>
 )}
 </div>
 </div>
 )}
 ...
 <RotateCcw className="h-5 w-5 text-blue" />
 Restore Project
 </DialogTitle>
 <DialogDescription>
