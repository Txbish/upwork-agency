...
const LINK_TYPE_ICONS: Record<string, React.ReactNode> = {
 [ProjectLinkType.GITHUB]: <Github className="h-4 w-4 text-storm/70 shrink-0" />,
 [ProjectLinkType.VERCEL]: <Globe className="h-4 w-4 text-storm/70 shrink-0" />,
 [ProjectLinkType.STAGING]: <Server className="h-4 w-4 text-storm/70 shrink-0" />,
 [ProjectLinkType.DOCS]: <BookOpen className="h-4 w-4 text-storm/70 shrink-0" />,
 [ProjectLinkType.OTHER]: <LinkIcon className="h-4 w-4 text-storm/70 shrink-0" />,
};
...
function Card({ children, className = '' }: { children: React.ReactNode; className?: string }) {
 return (
 <div
 className={`rounded-card border border-mist bg-parchment/50 p-5 ${className}`}
 >
 {children}
 </div>
 );
...
export default function ProjectDetailPage() {
 ...
 <div className="flex flex-col items-center justify-center py-20 text-storm/70">
 ...
 <h1 className="text-[32px] lg:text-[40px] font-medium leading-[1.05] tracking-[-0.025em] text-ink">{project.title}</h1>
 <Badge variant={STAGE_VARIANT[project.stage] ?? 'outline'}>
 {STAGE_LABELS[project.stage] ?? project.stage}
 </Badge>
 </div>
 <div className="flex flex-wrap items-center gap-3 text-sm text-storm/70">
 {project.clientName && (
 ...
 {showTaskForm && (
 <div className="mb-4 space-y-3 rounded-lg border border-mist bg-muted/30 p-4">
 <div>
 ...
 {/* Mini kanban */}
 {allTasks.length === 0 ? (
 <p className="text-sm text-storm/70 py-4 text-center">No tasks yet</p>
 ) : (
 ...
 {milestones.length > 0 && (
 <div className="mb-4">
 <div className="flex items-center justify-between text-xs text-storm/70 mb-1">
 <span>
 ...
 <div
 className="bg-ink rounded-full h-2 transition-all"
 style={{
 ...
 {/* Milestone list */}
 {milestones.length === 0 ? (
 <p className="text-sm text-storm/70 py-4 text-center">No milestones yet</p>
 ) : (
 ...
 milestone.completed
 ? 'border-[hsl(var(--success))]/35 bg-cream/80'
 : 'border-mist/70 bg-cream/50'
 }`}
 >
 {milestone.completed ? (
 ...
 milestone.completed ? 'line-through text-storm/70' : ''
 }`}
 >
 {milestone.name}
 </span>
 {milestone.description && (
 <p className="text-xs text-storm/70 truncate">
 {milestone.description}
 </p>
 )}
 </div>
 {milestone.dueDate && (
 <span className="text-xs text-storm/70 shrink-0 flex items-center gap-1">
 ...
 {meetings.length === 0 ? (
 <p className="text-sm text-storm/70 py-4 text-center">
 ...
 <th className="text-left py-2 px-2 text-xs font-semibold uppercase tracking-wider text-storm/70">
 Date
 </th>
 <th className="text-left py-2 px-2 text-xs font-semibold uppercase tracking-wider text-storm/70">
 Type
 ...
 .map((meeting) => (
 <tr key={meeting.id} className="border-b border-mist/30 last:border-b-0">
 ...
 <td className="py-2.5 px-2 text-storm/70">
 {formatUserName(meeting.closer)}
 </td>
 <td className="py-2.5 px-2 text-storm/70 max-w-xs">
 {meeting.notes || '\u2014'}
 </td>
 <td className="py-2.5 px-2">
 ...
 !meeting.loomUrl &&
 !meeting.driveUrl && (
 <span className="text-xs text-storm/70">{'\u2014'}</span>
 )}
 </div>
 </td>
 </tr>
 ...
 <div className="rounded-lg border border-mist/70 bg-cream/50 p-3 text-center">
 <p className="text-xs text-storm/70 uppercase tracking-wider">
 Hours Billed
 </p>
 ...
 <span className="text-xs text-storm/70 ml-1">override</span>
 )}
 </p>
 </div>
 {canManageTasks(role) && (
 ...
 <span className="text-storm/70">Contract Value</span>
 ...
 <Separator />
 <div className="flex justify-between">
 <span className="text-storm/70">Pricing</span>
 ...
 <Separator />
 <div className="flex justify-between">
 <span className="text-storm/70">Milestones</span>
 <span className="font-medium">
 ...
 {showLinkForm && (
 <div className="mb-4 space-y-3 rounded-lg border border-mist bg-muted/30 p-3">
 <Input
 placeholder="Label"
 value={newLinkLabel}
 ...
 <p className="text-sm text-storm/70 py-2 text-center">No links added</p>
 ) : (
 ...
 <h3 className="text-xs font-semibold uppercase tracking-wider text-storm/70">
 ...
 <div
 key={link.id}
 className="flex items-center gap-2 rounded-lg border border-mist/70 bg-cream/50 px-3 py-2"
 >
 <a
 href={link.url}
 target="_blank"
 rel="noopener noreferrer"
 className="text-sm font-medium text-blue hover:underline truncate flex-1"
 >
 {link.label}
 </a>
 {canManageLinks(role) && (
 ...
 size="sm"
 variant="ghost"
 className="h-7 w-7 p-0 text-storm/70 hover:text-destructive shrink-0"
 ...
 <p className="text-sm text-storm/70 whitespace-pre-wrap">
 ...
 <div className="flex items-center justify-between mb-4">
 <div className="flex items-center gap-2">
 <FileCode className="h-5 w-5 text-blue" />
 ...
 <div
 key={msg.id}
 className="rounded-lg border border-mist/70 bg-cream/50 p-3 space-y-1"
 >
 <div className="flex items-center gap-2">
 ...
 <span className="text-xs text-storm/70 ml-auto">
